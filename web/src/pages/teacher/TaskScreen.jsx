      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px 24px' }}>
        ...
        <div style={{ background: 'var(--white)', border: '1px solid var(--ink-200)', borderRadius: 12, overflow: 'hidden' }}>
          {task.submissions.map((s, i) => (
            <button key={s.id} onClick={() => nav(`/profesor/clases/${classId}/tareas/${taskId}/calificar/${s.id}`)} style={{
              width: '100%', padding: '12px 14px', border: 0, borderTop: i === 0 ? 'none' : '1px solid var(--ink-200)',
              background: 'transparent', cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 12,
            }}>
              ...
              {s.grade != null ? (
                <div style={{ padding: '5px 10px', borderRadius: 999, background: 'var(--success-100)', color: '#1a6b47', fontFamily: 'var(--font-mono)', fontWeight: 800, fontSize: 12 }}>{s.grade}/100</div>
