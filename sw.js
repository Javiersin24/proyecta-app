...
import { gbFmtIn, gbColorIn, DEFAULT_SCALE } from '../../lib/gradebook.js';

const PALETTE = ['var(--indigo-500)', 'var(--coral-500)', '#0EA5A0', '#8B5CF6', '#D99400'];
// Cada materia puede usar su propia escala (0–5.0 ó 0–100), así que las notas
// se muestran siempre con la escala de su clase.
const scaleOf = (o) => (o?.scale?.max > 0 ? o.scale : DEFAULT_SCALE);
const etiquetaEscala = (s) => (s.max === 5 ? '0–5.0' : `0–${s.max}`);
...
export default function StudentCalificacionesScreen() {
  ...
  if (!selId) {
    // Si todas las materias comparten escala, el promedio se muestra en ella;
    // si hay escalas distintas se normaliza a porcentaje para no mezclar peras
    // con manzanas.
    const conNota = rows.filter((r) => r.definitiva != null);
    const escalas = [...new Set(rows.map((r) => scaleOf(r).max))];
    const mismaEscala = escalas.length <= 1;
    const escalaGeneral = mismaEscala ? scaleOf(rows[0]) : { max: 100, pass: 60 };
    const promedioGeneral = conNota.length
      ? (mismaEscala
        ? conNota.reduce((a, r) => a + r.definitiva, 0) / conNota.length
        : (conNota.reduce((a, r) => a + (r.definitiva / scaleOf(r).max) * 100, 0) / conNota.length))
      : null;
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '4px 16px 24px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'var(--indigo-50)', border: '1px solid var(--indigo-100)', borderRadius: 16, padding: '16px 18px' }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 12.5, color: 'var(--fg-3)', marginTop: 2 }}>{rows.length} materias · {mismaEscala ? `escala ${etiquetaEscala(escalaGeneral)}` : 'escalas distintas → % '}</div>
            </div>
            <div style={{ fontSize: 34, fontWeight: 800, lineHeight: 1, color: gbColorIn(promedioGeneral, escalaGeneral) }}>{gbFmtIn(promedioGeneral, escalaGeneral)}</div>
          </div>
              ...
              <table style={{ borderCollapse: 'collapse', width: '100%', fontSize: 13 }}>
                ...
                <tbody>
                  {rows.map((r, i) => {
                    ...
                    const estado = r.definitiva == null ? { t: 'Sin notas', bg: 'var(--ink-100)', fg: 'var(--fg-3)' } : r.definitiva >= scaleOf(r).pass ? { t: 'Aprobada', bg: '#DCFCE7', fg: '#1a6b47' } : { t: 'En riesgo', bg: '#FEE2E2', fg: '#B42318' };
                    return (
                      <tr key={r.classId} onClick={() => setSelId(r.classId)} style={{ cursor: 'pointer', borderTop: i ? '1px solid var(--ink-100)' : 0 }}>
                        ...
                        <td style={{ padding: '11px 12px', textAlign: 'right', fontSize: 18, fontWeight: 800, color: gbColorIn(r.definitiva, scaleOf(r)) }}>{gbFmtIn(r.definitiva, scaleOf(r))}</td>
  ...
  const detScale = scaleOf(detail) || scaleOf(selClass);
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, background: 'var(--indigo-50)', border: '1px solid var(--indigo-100)', borderRadius: 16, padding: '16px 18px' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12.5, color: 'var(--fg-3)', marginTop: 2 }}>escala {etiquetaEscala(detScale)} · aprueba con {gbFmtIn(detScale.pass, detScale)}</div>
              </div>
              <div style={{ fontSize: 38, fontWeight: 800, lineHeight: 1, color: gbColorIn(detail.definitiva, detScale) }}>{gbFmtIn(detail.definitiva, detScale)}</div>
            </div>
            {detail.cats.map((cat) => (
              <div key={cat.id} style={{ background: 'var(--white)', border: '1px solid var(--border-subtle)', borderRadius: 16, overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderBottom: '1px solid var(--ink-100)', background: 'var(--paper-50)' }}>
                  ...
                  <span style={{ fontSize: 17, fontWeight: 800, color: gbColorIn(cat.avg, detScale), minWidth: 38, textAlign: 'right' }}>{gbFmtIn(cat.avg, detScale)}</span>
                </div>
                <div>
                  {cat.cols.map((col, i) => (
                    <div key={col.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 16px', borderTop: i ? '1px solid var(--ink-100)' : 0 }}>
                      ...
                      <span style={{ fontSize: 15, fontWeight: 700, color: gbColorIn(col.val, detScale), minWidth: 38, textAlign: 'right' }}>{gbFmtIn(col.val, detScale)}</span>
                    </div>
                  ))}
                </div>
              </div>
