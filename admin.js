...
// Nota consolidada del colegio. Solo puede escribirla el profesor que
// EFECTIVAMENTE dicta esa materia en ese grupo — no basta con ser profesor del
// mismo colegio, o cualquiera podría alterar notas de clases ajenas.
router.put('/groups/:groupId/grade', async (req, res) => {
  ...
  const dicta = await prisma.groupSchedule.findFirst({
    where: { groupId: group.id, materia, teacherId: req.user.id },
    select: { id: true },
  });
  if (!dicta) return res.status(403).json({ error: 'No dictas esta materia en este grupo' });
